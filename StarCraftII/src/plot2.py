...
#algos = ['our attack', 'baselines'] # 'attack1 ICLR',
algos = ['our attack']
...
#        print(max_means)
        for i in range(len(algos)):
            ax.hlines(max_means[0], 0, 2e7, colors = colors[i], ls = '--')
            ax.text(0, max_means[0], '%.2f'%max_means[0], fontsize=24, verticalalignment='bottom', color = colors[0], fontweight = 'bold')
...
#        ax.hlines(t, 0, 2e7, colors = 'green', ls = '--')
        ...
        #plt.xlabel('')
        #plt.ylabel('')
        plt.xticks([0, 0.5e7, 1.0e7, 1.5e7, 2.0e7], size=21, fontweight = 'bold')
        plt.yticks(size=21, fontweight = 'bold')
        ...
        sc_env = 'sc'
        plt.savefig('{}_{}.pdf'.format(sc_env, shuffix), bbox_inches='tight')
        plt.close()
