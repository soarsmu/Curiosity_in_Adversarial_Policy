...
class PPO_AdvActor(object):
    ...
    def __init__(self, env, policy, value, unroll_length, gamma, lam, queue_size=1,
                 ...
                 reward_shape='none', use_victim_ob=False, victim_model=None, norm_model_path=None, 
                 mix_ratio=0.5):
        """
        ...
        for _ in range(self._unroll_length):
            ...
            if self._done:
                ...
                episode_infos.append({'r': self._cum_reward, 'win': int(info['winning']), 'tie': int(info['tie']),
                                      'loss': int(info['loss']), 'is_advagent': int(self.is_advagent)})
                
                ## load the model
                self.is_advagent = (random.uniform(0, 1) < self.mix_ratio)

                if not self.is_advagent:
                    self._oppo_model.load(self.norm_model_path)
                else:
                    self._oppo_model.load(self.victim_model)
  ...
  def run(self):
    ...
    while True:
      ...
      if updates % self._print_interval == 0:
        ...
        win_adv_game = sum([info['win'] * info['is_advagent'] for info in self._episode_infos])
        tie_adv_game = sum([info['tie'] * info['is_advagent'] for info in self._episode_infos])
        loss_adv_game = sum([info['loss'] * info['is_advagent'] for info in self._episode_infos])
        total_adv_game = sum(info['is_advagent'] for info in self._episode_infos)

        win_norm_game = win_game - win_adv_game
        tie_norm_game = tie_game - tie_adv_game
        loss_norm_game = loss_game - loss_adv_game
        total_norm_game = total_game - total_adv_game
        ...
        adv_winning_rate = adv_win_count_tie = adv_win_plus_tie = -1.0

        if total_adv_game > 0:
            adv_winning_rate = win_adv_game * 1.0 / total_adv_game
            adv_win_count_tie = (((win_adv_game - loss_adv_game) * 1.0 / (total_adv_game)) + 1) / 2.0
            adv_win_plus_tie = (win_adv_game + tie_adv_game) * 1.0 / (total_adv_game)        

        norm_winning_rate = norm_win_count_tie = norm_win_plus_tie = -1.0

        if total_norm_game > 0:
            norm_winning_rate = win_norm_game * 1.0 / total_norm_game 
            norm_win_count_tie = (((win_norm_game - loss_norm_game) * 1.0 / (total_norm_game)) + 1) / 2.0
            norm_win_plus_tie = (win_norm_game + tie_norm_game) * 1.0 / (total_norm_game)
        ...
        if total_adv_game > 0:
            tprint('Total_Adv_Game is %d, Adv_Winning_rate is %f, Adv_Winning_rate_tie is %f, Adv_Winning_plus_tie is %f,'
                   'adv_win %d, adv_tie %d, adv_loss %d,'
                    % (total_adv_game, adv_winning_rate, adv_win_count_tie,  adv_win_plus_tie, win_adv_game, tie_adv_game, loss_adv_game))

        if total_norm_game > 0:
            tprint('Total_Norm_Game is %d, Norm_Winning_rate is %f, Norm_Winning_rate_tie is %f, Norm_Winning_plus_tie is %f,'
                   'norm_win %d, norm_tie %d, norm_loss %d,'
                    % (total_norm_game, norm_winning_rate, norm_win_count_tie,  norm_win_plus_tie, win_norm_game, tie_norm_game, loss_norm_game))

        if self._save_dir is not None:
            os.makedirs(self._save_dir, exist_ok=True)
            fid = open(self._save_dir + '/Log.txt', 'a+')
            fid.write("%d %f %f %f %f %f %f %f %f %f\n" %(updates, winning_rate, win_count_tie, win_plus_tie, adv_winning_rate, \
                                                 adv_win_count_tie, adv_win_plus_tie, norm_winning_rate, norm_win_count_tie, norm_win_plus_tie))
            fid.close()
