...
def plot_graph(cluster_ids_path, output_dir):
    ...
    colors = {"Our": "blue", "Zoo": 'yellow',  "Rnd": "red"} # "Ucb": "blue",
    len = int(data.shape[0] / 3)

    opponent_type =  ['Zoo'] * len + ['Our'] * len +  ['Rnd'] * len
    ...
    ax.scatter(data[0: len - 1, 0], data[0: len - 1, 1], c=hues[0: len - 1], marker='o', alpha=0.75, s=0.4, edgecolors="none", linewidth=1)
    ax.scatter(data[len: 2 * len - 1, 0], data[len: 2 * len - 1, 1], c=hues[len: 2 * len - 1], marker='o', alpha=0.75, s=0.4, edgecolors="none", linewidth=1)
    ax.scatter(data[2 * len: 3 * len - 1, 0], data[2 * len: 3 * len - 1, 1], c=hues[2 * len: 3 * len - 1], marker='o', alpha=0.75, s=0.4, edgecolors="none", linewidth=1)
    fig.savefig(output_dir + '/' + output_dir.split('/')[-1] + '.pdf')

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument("--dir", type=str, default='../activations/tsne/SumoAnts')
    parser.add_argument("--output_dir", type=str, default='../activations/tsne/SumoAnts')
