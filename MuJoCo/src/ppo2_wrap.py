...
from value import MlpLstmValue, MlpValue, RndValue
import pdb

# class RNDBaseNetwork():
...
class MyPPO_RND(ActorCriticRLModel):
    """ Learn policy with PPO """
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.2,
        ...
        super(MyPPO_RND, self).__init__(policy=policy, env=env, verbose=verbose, requires_vec_env=True,
                                     _init_setup_model=_init_setup_model, policy_kwargs=policy_kwargs)
        self.coef_opp_init = coef_opp_init
        self.coef_opp_schedule = coef_opp_schedule
        self.coef_adv_init = coef_adv_init
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                if self.retrain_vicitim and not self.use_baseline_policy:
                    ...
                    # ret_variable = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="victim_policy/retfilter")
                    obs_variable = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="victim_policy/obsfilter")
                    variables = obs_variable + tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                                                 scope="victim_policy")
                    setFromFlat(variables, param, self.sess)
    ...
    def _train_step(self, learning_rate, cliprange, coef_opp, coef_adv, coef_abs, explore_cof, obs, returns, masks, actions, values,
        ...
        intrinsic_advs = intrinsic_returns - intrinsic_values
        ...
        if self.retrain_vicitim and not self.use_baseline_policy:
            ...
            td_map = {self.train_model.obs_ph: obs, self.action_ph: actions, self.advs_ph: advs,
                      ...
                      self.opp_advs_ph: opp_advs,
                      self.opp_rewards_ph: opp_returns,
                      self.old_opp_vpred_ph: opp_values,
                      self.abs_advs_ph: abs_advs,
                      self.abs_rewards_ph: abs_returns,
                      self.old_abs_vpred_ph: abs_values,
                      self.coef_opp_ph: coef_opp,
                      self.coef_adv_ph: coef_adv,
                      self.coef_abs_ph: coef_abs,
                      self.vtrain1_model.obs_ph: opp_obs,
                      self.vtrain_model.obs_ph: opp_obs,
                      self.intrinsic_advs_ph: intrinsic_advs,
                      self.intrinsic_rewards_ph: intrinsic_returns,
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO_RND",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, nupdates + 1):
                ...
                # true_reward is the reward without discount
                obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward, opp_true_reward, \
                abs_true_reward, opp_obs, opp_states, opp_returns, opp_values, abs_states, abs_returns, abs_values, intrinsic_return, intrinsic_value, predict_feature \
                    = runner.run()
                # obs (n_envs*n_steps, 137), returns (n_envs*n_steps, ), masks (n_envs*n_steps, ), actions (n_envs*n_steps, ac_space), values (n_envs*n_steps, ), neglogpacs (n_envs*n_steps, ), states (n_envs, 512), true_reward, opp_true_reward, abs_true_reward, opp_obs, opp_states, opp_returns, opp_values, abs_states, abs_returns, abs_values
                ep_info_buf.extend(ep_infos)
                mb_loss_vals = []
                ...
                if self.is_mlp and states is None:  # is_mlp used to decide the type of opponent value function model and state is used to decide adversarial networks type.
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_batch, batch_size):
                            ...
                            mbinds = inds[start:end]
                            slices = (arr[mbinds] for arr in (obs, returns, masks, actions, values, neglogpacs))
                            slices_victim = (arr[mbinds] for arr in
                                             (opp_obs, opp_returns, opp_values, abs_returns, abs_values))
                            slice_intrinsic_return = (arr[mbinds] for arr in (intrinsic_return, intrinsic_value, predict_feature))
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_envs, envs_per_batch):
                            ...
                            slices = (arr[mb_flat_inds] for arr in
                                      (obs, returns, masks, actions, values, neglogpacs))
                            slices_victim = (arr[mb_flat_inds] for arr in
                                             (opp_obs, opp_returns, opp_values, abs_returns, abs_values))
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        mb_opp_obs, mb_opp_returns, mb_opp_values, mb_abs_returns, mb_abs_values, mb_intrinsic_returns, mb_intrinsic_value, mb_predicted_features = \
            map(swap_and_flatten, (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward, opp_true_reward, \
                                   ...
                                   mb_intrinsic_returns, mb_intrinsic_value, mb_predicted_features))
