...
parser.add_argument("--adv_path", type=str, default='/home/gc/Curiosity_in_Adversarial_Policy-review/MuJoCo/our agent/attack/sumohumans/model.pkl')
parser.add_argument("--adv_ismlp", type=bool, default=True)
# adversarial agent's observation norm mean / variance path
parser.add_argument("--adv_obs_normpath", type=str, default='/home/gc/Curiosity_in_Adversarial_Policy-review/MuJoCo/our agent/attack/sumohumans/obs_rms.pkl')
...
if __name__=="__main__":
        ...
        venv = SubprocVecEnv([lambda: make_mixadv_multi2single_env(env_name, VIC_AGT_ID, ADV_AGENT_PATH, ADV_AGENT_NORM_PATH,
                                                  REW_SHAPE_PARAMS, scheduler, ADV_ISMLP,
                                                  reverse=REVERSE, ratio=MIX_RATIO, total_step=TRAINING_ITER, is_rnd=args.is_rnd) for i in range(N_GAME)])
        # test
        if REVERSE:
...
#        model = MyPPO_RND(baseline_policy,
#                          venv,
#                          lr_schedule=LR_SCHEDULE,
#                          coef_opp_init=COEF_VIC_INIT,
#                          coef_opp_schedule=COEF_VIC_SCHEDULE,
