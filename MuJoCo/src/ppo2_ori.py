        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.old_opp_vpred_ph = tf.placeholder(tf.float32, [None],
                                                           name='old_opp_vpred_ph')  # (n_step*n_envs//minibatch)
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        if self.use_victim_ob:
            opp_last_values, _ = self.v_model.value(np.asarray(self.env.get_attr('oppo_ob_next').copy()),
                                                    self.opp_states, self.dones)
            abs_last_values, _ = self.v1_model.value(np.asarray(self.env.get_attr('oppo_ob_next').copy()),
                                                     self.abs_states, self.dones)
        else:
        ...
        for step in reversed(range(self.n_steps)):
            ...
            mb_opp_advs[
                step] = opp_last_gae_lam = opp_delta + self.gamma * self.lam * nextnonterminal * opp_last_gae_lam

            abs_delta = mb_abs_rewards[step] + self.gamma * abs_nextvalues * nextnonterminal - mb_abs_values[step]
            mb_abs_advs[
                step] = abs_last_gae_lam = abs_delta + self.gamma * self.lam * nextnonterminal * abs_last_gae_lam
        ...
        mb_opp_obs, mb_opp_returns, mb_opp_values, mb_abs_returns, mb_abs_values = \
            map(swap_and_flatten,
                (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward, opp_true_reward, \
